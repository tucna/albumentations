...
class UnpropShuffle(DualTransform):
    ...
    def apply(self, img, tiles=None, **params):
        if tiles is None:
            tiles = []
        self.moje = self.shuffled_ids.copy()
    ...
    def apply_to_bboxes(self, bboxes, **params):
        ...
        rows = params["rows"]
        cols = params["cols"]
        return F.unprop_bbox_transpose(bboxes, tiles, self.moje, rows, cols)
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        while current_refinement < refinement_steps:
            if not refinement():
                break
            else:
